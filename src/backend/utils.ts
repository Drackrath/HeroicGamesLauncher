...
function sendGameStatusUpdate(payload: GameStatus) {
  ...
  // Focus the game window when status changes to 'playing'
  if (payload.status === 'playing') {
    focusGameWindow()
  }
}

let activeFocusProcess: import('child_process').ChildProcess | null = null

/**
 * Polls for a new game window and brings it to the foreground.
...
function focusGameWindow() {
  ...
  try {
    ...
    if (isWindows) {
      ...
      child = spawn('xdotool', ['search', '--onlyvisible', '--class', '.', 'windowactivate'], { timeout: 3000, stdio: 'ignore' })
    } else if (isMac) {
      child = spawn('osascript', ['-e', 'tell application "System Events" to set frontmost of (first process whose visible is true and name is not "Heroic") to true'], { timeout: 3000, stdio: 'ignore' })
    } else {
      return
    }
